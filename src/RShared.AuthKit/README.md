...
AuthKit — **только аутентификация**: он доказывает, что человек владеет внешним аккаунтом (логин+пароль, Google, Telegram), и выдаёт сессию. Хранилище юзеров, роли и права остаются на стороне потребителя — через швы:
...
IOneTimeCodeStore       одноразовые коды (дефолт — память процесса)
```
...
Механизм общий, не только для Telegram: одноразовый секрет с **purpose** (`Login` / `EmailConfirm` / `PasswordReset`), **каналом** (email/telegram) и **назначением** (адрес или TG-юзер-id). Код чужого purpose никогда не сработает в другом потоке — совпадает вся тройка. Повторная выдача кода той же тройке аннулирует предыдущий: активный код на назначение — один. Своё хранилище кодов (Redis, таблица в БД) реализует `IOneTimeCodeStore` и регистрируется до `AddAuthKit`.
...
- одноразовые коды по умолчанию в памяти процесса (`MemoryOneTimeCodeStore`) — для много-нодового деплоя зарегистрировать свою реализацию `IOneTimeCodeStore` до `AddAuthKit`;
