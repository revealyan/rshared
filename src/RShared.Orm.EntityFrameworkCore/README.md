...
## Регистрация контекста: четыре способа EF Core

Контекст регистрируешь ты, любым из четырёх способов — реестр подстраивается сам:

| способ | чем является | как подхватывается реестром |
|---|---|---|
| `AddDbContext<T>` | scoped-контекст на скоуп | прямой резолв `T` из скоупа |
| `AddDbContextPool<T>` | pooled scoped | тот же прямой резолв (пул выдаёт арендованный на скоуп экземпляр) |
| `AddDbContextFactory<T>` | singleton-фабрика короткоживущих контекстов | ветка `IDbContextFactory<T>`: контекст создаётся на скоуп, диспозит фабрика репозиториев |
| `AddPooledDbContextFactory<T>` | фабрика + пул | та же фабричная ветка |

Дефолт для веба с UoW — первый (`AddDbContext`): скоуп = единица работы, один трекер на реквест.
Пул — экономия аллокаций, контекст обязан быть без собственного стейта. Фабричные — фоновые
воркеры и параллельные единицы работы. Под постгрес всё это есть одной опцией —
`RShared.Orm.PostgreSql` → `ContextRegistration`.
