...
- `.github/workflows/ci.yml` — единый конвейер: PR — build и test; мердж в main — build → test → publish (версия: major.minor из `version.json`, патч = последняя в фиде + 1, после публикации тег `v{версия}`). Релизы только через МР.
